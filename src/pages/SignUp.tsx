...
export const SignUp = () => {
  ...
  const onSendCode: SubmitHandler<FormValues> = async (data) => {
    setLoading(true);
    try {
  ...
  const onSignUp: SubmitHandler<FormValues> = async (data) => {
    ...
    try {
      ...
      const axiosErr = err as AxiosError<{ error?: string }>;
      const msg = axiosErr.response?.data?.error || "Failed to sign up";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md bg-white rounded-lg shadow-md p-8">
        ...
        <form
          onSubmit={
            codeSent ? handleSubmit(onSignUp) : handleSubmit(onSendCode)
          }
          className="space-y-4"
        >
          <div>
